...
public class EGLview extends GLSurfaceView implements View.OnTouchListener, View.OnKeyListener, View.OnLongClickListener {
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			if(longPress == true){
				...
				GEarth.touchPicker(touchID, event.getX(0), event.getY(0));
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			for(int i=0; i<n_points; i++){
				...
				GEarth.touchBeganEvent(touchID, event.getX(i), event.getY(i));
			}
			break;
		case MotionEvent.ACTION_CANCEL:
			for(int i=0; i<n_points; i++){
				int touchID = event.getPointerId(i);
				GEarth.touchEndedEvent(touchID, event.getX(i), event.getY(i),1);
			}
			break;
				...
				if(n_points > 1){ 
					for (int h = 0; h < historySize; h++) {
						...
						for (int p = 0; p < n_points; p++) {
							...
							GEarth.touchMovedEvent(touchID, event.getHistoricalX(p, h), event.getHistoricalY(p, h));
						}
					}
				}
				for(int i=0; i<n_points; i++){
					int touchID = event.getPointerId(i);
					GEarth.touchMovedEvent(touchID, event.getX(i), event.getY(i));
				}
				break;
		...
		case MotionEvent.ACTION_POINTER_UP:
			for(int i=0; i<n_points; i++){
				int touchID = event.getPointerId(i);
				GEarth.touchEndedEvent(touchID, event.getX(i), event.getY(i), tapcount);
				tapcount = 0;
			}
			break;
		default :
	...
	private class GestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onDown(MotionEvent event) {
			Log.d(TAG, "onDown");
			int touchID = event.getPointerId(0);				
			GEarth.touchPosition(touchID, event.getX(0), event.getY(0));
			return true;
		}
