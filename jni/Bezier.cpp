...
void Bezier::getBezier(const std::vector<GVCoord>& controlPoints, std::vector<GVCoord>* bezierPoints)
{
	...
	if(num >= 2)
	{
		for(int i = 1,j=0; i <= num-1; i++,j++)
		{
			...
			if(fabs(lon_interpolation[j]) > 180)
			{
...
void Bezier::getBezier2(const std::vector<GVCoord>& controlPoints, std::vector<GVCoord>* bezierPoints)
{
	for(int i=0; i < controlPoints.size(); i++)
	{
		bezierPoints->push_back(GVCoord(controlPoints[i].lon, controlPoints[i].lat, 0));
	}
